...
Installation requires every launcher lifecycle to be idle and all open drafts and pending runtime arguments to be saved or cleared. The app pauses editing during installation and obtains an exact-owner mutation reservation before invoking Homebrew. That private reservation survives service replacement and remains held while the installing app process is alive, with a 15-minute recovery grace if it exits unexpectedly. A failed install releases the reservation and restores editing; an older daemon that cannot persist the reservation must be upgraded through the trusted installer first. Service recovery also accepts a custom app location only when the existing LaunchAgent contract and bundle signature validate against the fixed publisher policy.
